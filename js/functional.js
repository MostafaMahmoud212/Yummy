export  class HtmlFunctional{
		...
		this.PageContent.innerHTML = `
			<div class="col-12 d-flex flex-wrap gap-3 justify-content-center pb-4">
				<div class="col-md-5 col-sm-12">
						<input id="nameSearch"  onkeyup="html_handler.searchName(this.value)"   type="text" placeholder="Search By Name" class="form-control bg-transparent text-white custom-serach-placeholder">
				</div>
				<div class="col-md-5 col-sm-12">
						<input id="firstLetterSearch"  onkeyup="html_handler.searchFirstLetter(this.value)" maxlength="1"  type="text" placeholder="Search By First letter" class="form-control bg-transparent text-white custom-serach-placeholder">
				</div>
			</div>
	...
	handleSidebar(){
						// Open Nav
		let navButtons = $(".sidebar .nav-content .nav-buttons button"); 
		let sidebar = document.getElementById('sidebar')
		let open_close_button = document.getElementById('openButton')
		if(sidebar.classList.contains("closed")){
			sidebar.style.transitionDelay = 0 + 's';
			for (let i = 0; i < navButtons.length; i++){
				let delay = i * 0.1;
				navButtons[i].style.transitionDelay = delay + 's';
			}
			...
			for (let i = 0;i<navButtons.length;i++){
				let delay = (navButtons.length - i) * 0.05;
				navButtons[i].style.transitionDelay = delay + 's';
			}
			...
			<div class="row py-5 g-4 " id="rowData"><div style="height: 70vh !important;" class="contact d-flex justify-content-center align-items-center">
				<div class="container w-75 text-center">
					<div class="row g-4">
						...
						<div class="col-md-6">
							<input id="phoneInput" onkeyup="html_validator.phoneValidator()" type="number" class="form-control " placeholder="Enter Your Phone" >
					...
					<button id="submitBtn" class="btn btn-outline-danger px-2 mt-3" type="submit">Submit</button>
				</div>
			</div>`
	}
}
